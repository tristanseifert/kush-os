    ...
    for(const auto &rel : rels) {
        ...
        switch(type) {
            ...
            case R_386_RELATIVE: {
                ...
                memcpy(&value, from, sizeof(uint32_t));
                value += base;
                memcpy(from, &value, sizeof(uint32_t));
                break;
            }
            ...
            case R_386_GLOB_DAT: {
                auto from = reinterpret_cast<void *>(base + rel.r_offset);
                memcpy(from, &symbol->address, sizeof(uint32_t));
                break;
            }
 ...
 */
void ElfReader::patchRelocsAmd64(const PaddedArray<Elf_Rel> &rels, const uintptr_t base) {
    const SymbolMap::Symbol *symbol;
    ...
    for(const auto &rel : rels) {
        ...
        switch(type) {
            ...
            /// unknown relocation type
            default:
                Linker::Abort("unsupported %s relocation: type %u (off %x info %x)", "amd64",
                        ELF64_R_TYPE(rel.r_info), rel.r_offset, rel.r_info);
        }
    }
}
