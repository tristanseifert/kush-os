 ...
 */
std::string Client::AddDevice(const std::string &parent, const std::string &driverId, const std::span<std::byte> &auxData) {
    uint32_t sentTag;
    {
        ...
        capnp::Data::Reader blobReader_auxData(reinterpret_cast<const kj::byte *>(auxData.data()), auxData.size());
        request.setAuxData(blobReader_auxData);
