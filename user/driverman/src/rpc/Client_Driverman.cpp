...
std::string Client::AddDevice(const std::string &parent, const std::string &driverId) {
    ...
    }
    {
        ...
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_ADD_DEVICE) this->_HandleError(false, "Invalid type in reply RPC packet");
 ...
 */
int32_t Client::SetDeviceProperty(const std::string &path, const std::string &key, const std::span<std::byte> &data) {
    uint32_t sentTag;
    {
        capnp::MallocMessageBuilder requestBuilder;
    ...
    }
    {
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) this->_HandleError(false, "Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_SET_DEVICE_PROPERTY) this->_HandleError(false, "Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));
...
Client::GetDevicePropertyReturn Client::GetDeviceProperty(const std::string &path, const std::string &key) {
    ...
    }
    {
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) this->_HandleError(false, "Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_GET_DEVICE_PROPERTY) this->_HandleError(false, "Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));
...
int32_t Client::StartDevice(const std::string &path) {
    ...
    }
    {
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) this->_HandleError(false, "Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_START_DEVICE) this->_HandleError(false, "Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));
...
int32_t Client::StopDevice(const std::string &path) {
    ...
    }
    {
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) this->_HandleError(false, "Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_STOP_DEVICE) this->_HandleError(false, "Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));
...
int32_t Client::Notify(const std::string &path, uint64_t key) {
    ...
    {
        capnp::MallocMessageBuilder requestBuilder;
        auto request = requestBuilder.initRoot<rpc::_proto::messages::NotifyRequest>();
        request.setPath(path);
        request.setKey(key);
        auto rw = capnp::messageToFlatArray(requestBuilder);
        auto rb = rw.asBytes();
        const std::span<std::byte> rs(reinterpret_cast<std::byte *>(rb.begin()), rb.size());
        sentTag = this->_sendRequest(rpc::_proto::messages::MESSAGE_ID_NOTIFY, rs);
    }
    {
        std::span<std::byte> buf;
        if(!this->io->receiveReply(buf)) this->_HandleError(false, "Failed to receive RPC reply");
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) this->_HandleError(false, "Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_NOTIFY) this->_HandleError(false, "Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));

        kj::ArrayPtr<const capnp::word> message(reinterpret_cast<const capnp::word *>(payload.data()),
                payload.size() / sizeof(capnp::word));
        capnp::FlatArrayMessageReader reader(message);
        auto reply = reader.getRoot<rpc::_proto::messages::NotifyResponse>();
        return reply.getStatus();
    }
}
