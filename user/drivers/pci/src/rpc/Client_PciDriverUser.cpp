 ...
 */
std::string Client::GetDeviceAt(const libdriver::pci::BusAddress &address) {
    uint32_t sentTag;
    {
    ...
    }
    {
        ...
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_GET_DEVICE_AT) throw std::runtime_error("Invalid type in reply RPC packet");
 ...
 */
uint32_t Client::ReadCfgSpace32(const libdriver::pci::BusAddress &address, uint16_t offset) {
    uint32_t sentTag;
    {
        capnp::MallocMessageBuilder requestBuilder;
    ...
    }
    {
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) throw std::runtime_error("Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_READ_CFG_SPACE32) throw std::runtime_error("Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));
 ...
 */
void Client::WriteCfgSpace32(const libdriver::pci::BusAddress &address, uint16_t offset, uint32_t value) {
    uint32_t sentTag;
    {
        capnp::MallocMessageBuilder requestBuilder;
    ...
    }
    {
        ...
        const auto hdr = reinterpret_cast<const MessageHeader *>(buf.data());
        if(hdr->tag != sentTag) throw std::runtime_error("Invalid tag in reply RPC packet");
        else if(hdr->type != rpc::_proto::messages::MESSAGE_ID_WRITE_CFG_SPACE32) throw std::runtime_error("Invalid type in reply RPC packet");
        const auto payload = buf.subspan(offsetof(MessageHeader, payload));
