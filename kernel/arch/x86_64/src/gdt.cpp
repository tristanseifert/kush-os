...
void gdt_init() {
    ...
    gdt_set_entry((GDT_KERN_CODE_SEG >> 3), 0x00000000, 0xFFFFFFFF, 0x9A, 0xAF);
    gdt_set_entry((GDT_KERN_DATA_SEG >> 3), 0x00000000, 0xFFFFFFFF, 0x92, 0xCF); // should be 0x4F?

    // User code and data segments
    gdt_set_entry((GDT_USER_CODE_SEG >> 3), 0x00000000, 0xFFFFFFFF, 0xFA, 0xAF);
    ...
    for(int i = 0; i < GDT_NUM_TSS; i++) {
        gdt_set_entry64((GDT_FIRST_TSS >> 3) + i, ((uint64_t) &gTss[i]),
...
void tss_set_esp0(void *ptr) {
    ...
    gdt_set_entry64((GDT_FIRST_TSS >> 3) + tssIdx, ((uint64_t) &gTss[tssIdx]),
